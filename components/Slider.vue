<template>
  <div ref="rootEl" class="fixed inset-0 z-20 bg-heading/80" :class="{ 'opacity-0 ': closing }">
    <button
      @click="close()"
      aria-label="Close"
      class="hocus:text-accent fixed right-6 top-6 z-10 flex h-8 w-8 text-white transition ph:right-4 ph:top-4"
    >
    ...
    <div ref="containerEl" class="swiper-container h-screen">
      ...
      >
        <button
          ...
          ref="prevSlideEl"
          class="hocus:text-accent pointer-events-auto absolute right-full mr-6 flex h-8 w-8 text-white transition"
        >
        ...
        <button
          ...
          ref="nextSlideEl"
          class="hocus:text-accent pointer-events-auto absolute left-full ml-6 flex h-8 w-8 text-white transition"
        >
